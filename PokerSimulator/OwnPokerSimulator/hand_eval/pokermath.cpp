...
{
  int value1 = card1.value_;
  int value2 = card2.value_;
...
/*2*/ 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 7
  ...
  size_t index1 = 14 - card1.value_;
  size_t index2 = 14 - card2.value_;
...
void getAmountPerSuit(int& clubs, int& diamonds, int& hearts, int& spades, const std::vector<Card>& cards)
{
  for(size_t i = 0; i < cards.size(); i++)
  {
    if(cards[i].suit_ == S_CLUBS) clubs++;
    else if(cards[i].suit_ == S_DIAMONDS) diamonds++;
    else if(cards[i].suit_ == S_HEARTS) hearts++;
    else if(cards[i].suit_ == S_SPADES) spades++;
  }
}
                                 ...
                                 , int numOpponents, int numSamples)
{
  ...
  {
    shuffleN(others, NUMOTHER, 5 + numOpponents * 2); //the two extra table cards, and the cards of all opponents
...
int eval7_index(const Card& card)
{
  ...
  int value = card.value_ + 13 * (int)card.suit_ - 2;
...
int eval7(const int* cards)
{
  ...
  if(!inited)
  {
      PokerEval2::InitializeHandRankingTables(); inited = true;
  }
...
int eval7_index(const Card& card)
{
  ...
  int v = (card.value_ - 2);
  int s = (int)card.suit_ * 13;
  return s + v;                   //return a value between 0 and 51 (2c = 0, 3c = 1, 4c = 2, ..., As = 51)
}
