...
std::string Combination::getName() const
{
  ...
  if(type == C_STRAIGHT_FLUSH && cards[0].value_ == 14) result = "Royal Flush";
  return result;
}
...
Card getFlush(const std::vector<Card>& cards)
{
  ...
  int s = spades.size() < 5 ? 0 : spades[0].value_;
  int d = diamonds.size() < 5 ? 0 : diamonds[0].value_;
  int h = hearts.size() < 5 ? 0 : hearts[0].value_;
  int c = clubs.size() < 5 ? 0 : clubs[0].value_;
...
bool checkTwoPair(Card result[5], const std::vector<Card>& sorted)
{
  ...
  for(size_t i = 0; i < sorted.size() && c < 5; i++)
  {
    if(sorted[i].value_ != values[0] && sorted[i].value_ != values[1])
    {
      result[c] = sorted[i];
      c++;
    }
...
bool checkStraight(Card result[5], const std::vector<Card>& sorted)
{
  ...
  {
    if(sorted[i].value_ == s - 0 && !result[0].isValid()) result[0] = sorted[i];
    if(sorted[i].value_ == s - 1 && !result[1].isValid()) result[1] = sorted[i];
    if(sorted[i].value_ == s - 2 && !result[2].isValid()) result[2] = sorted[i];
    if(sorted[i].value_ == s - 3 && !result[3].isValid()) result[3] = sorted[i];
    if(sorted[i].value_ == s - 4 && !result[4].isValid()) result[4] = sorted[i];
  }
...
bool checkStraightFlush(Card result[5], const std::vector<Card>& sorted)
{
  ...
  {
    if(sorted[i].value_ + j == (size_t)c.value_ && sorted[i].suit_ == c.suit_)
    {
      result[j] = sorted[i];
      j++;
    }
...
int compare(const Combination& a, const Combination& b)
{
  ...
  {
    if(a.cards[i].value_ > b.cards[i].value_) return +1;
    if(a.cards[i].value_ < b.cards[i].value_) return -1;
  }
